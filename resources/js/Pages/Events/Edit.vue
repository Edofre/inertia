<template>
    <div>
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Edit #{{ event.id }}</h1>
            <div class="btn-toolbar mb-2 mb-md-0">
                <div class="btn-group mr-2">
                    <inertia-link class="btn btn-sm btn-outline-secondary" :href="$route('events.index')">Index</inertia-link>
                    <inertia-link class="btn btn-sm btn-outline-primary" :href="$route('events.show', [event.id])">Show</inertia-link>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            submit() {
                this.$inertia.put(this.$route('events.update', this.event.id), {
                    name: this.name,
